...
async function sendAuthenticatedRequest(options: {
  session: SessionType;
  apiBaseUrl: string;
  path: ApiPath;
  method: "GET" | "POST";
  ...
  if (!accessToken) {
    return {
      response: new Response(JSON.stringify({ error: "No active session. Please log in first." }), {
        status: 401,
...
async function sendApiRequest(options: {
  ...
  const url = `${options.apiBaseUrl}${options.path}`;
  const isBrandExtract = options.path === API_ROUTES.brand.extract;

  if (isBrandExtract) {
    console.info("[brand-extraction] dispatch", {
      url,
  ...
  try {
    ...
    if (isBrandExtract) {
      console.info("[brand-extraction] response", {
    ...
    if (isBrandExtract) {
      console.error("[brand-extraction] network-error", {
        url,
